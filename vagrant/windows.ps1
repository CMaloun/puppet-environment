...
if (!($PuppetInstalled)) {
  ...
  $install_args = @("/qn", "/norestart","/i", $MsiUrl, "PUPPET_AGENT_CERTNAME=agent-windows.talentsoft.com")
  ...
  ac -Encoding UTF8  "$($env:windir)\system32\Drivers\etc\hosts" "10.100.1.42 puppet"
  Write-Host "Host file updated"

  Write-Host "set environment variable"
  [Environment]::SetEnvironmentVariable("FACTER_role", "iiswebserver", "User")
